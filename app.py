...
# Main Navigation Tabs (এখন শুধুমাত্র ২টি ট্যাব আছে)
tab1, tab2 = st.tabs([
...
with tab1:
    ...
    with st.form("prediction_form"):
        ...
        with col1:
            ...
            # New Feature 1: Midterm Score (%)
            user_input["Midterm_Score"] = st.slider("📝 Midterm Marks (%)", min_value=0.0, max_value=100.0, value=65.0, step=1.0)
        ...
        with col2:
            ...
            # New Feature 2: Quiz Score (%)
            user_input["Quiz_Score"] = st.slider("✏️ Quiz Average Marks (%)", min_value=0.0, max_value=100.0, value=85.0, step=1.0)
    ...
    if submitted:
        try:
            ...
            st.error("Prediction failed. Please make sure you have generated updated .pkl files using train_model.py")
            st.code(str(e))
...
with tab2:
    ...
    st.write("This section provides insights into the most influential features affecting the model's predictions.")
    ...
    try:
        ...
        st.error("Error calculating Feature Importance.")
        st.code(str(e))